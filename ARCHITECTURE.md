...
### 19.6 Low-end pass (24 Sep; binding over §12 and §15 where they differ)
- Levels are `high`, `medium`, `low`, `minimal` (0.25 MP, no shadows; `settings.quality` accepts `'minimal'`, UI label "En düşük"). The canvas is created with `antialias: false`; post (outlines, grain) runs at every level, so scene programs never change with the level and setup always compiles against `post.target`. MSAA is post's target samples only (4 on high).
- `settings.quality: 'auto'` is `src/render/autoQuality.js`: a level plus a render scale (`renderer.setQuality(level, scale)`) follow wall-clock rAF deltas in every phase with a 3D view (not in 'setup', not while `loop.timeScale != 1` or the context is lost). Target 55 fps; a drop is sized by a pixels × level-cost model and may skip levels; a drop that gains nothing is undone; raises are slow probes, never above the start level, and cross into shadowed levels only in title/menu/results/podium. Fixed levels render at scale 1 and are never changed by auto.
- A software rasteriser (`renderer.software`, from UNMASKED_RENDERER_WEBGL) starts the session at `minimal` even over a fixed choice (the stored choice is kept), auto's ceiling is then `low`, and a closable hint (`data-kk-hint="hwaccel"`, dismissal in `uiHints.hwaccel`) points at the browser's hardware acceleration setting.
- The minimap writes dot transforms only on change; on `minimal` it moves them at 10 Hz.
- Measurement: `node tools/scenarios/lowend.mjs [--mode soft-1366,soft-phone,gpu-cpu6]`. Reference (24 Sep, before → after): soft-1366 race 2.6 → 19-20 fps, title 3.3 → 60; soft-phone race 2.9 → 20-22; gpu-cpu6 race 53-55 unchanged. The software floor (~20 fps at 0.06-0.1 MP) is Chrome's software compositing plus track/scenery shading.
- Follow-up to 19.5 (verification round 2): a blur/hide or context loss during 'setup' or 'intro' is latched and pauses the game when the countdown starts (a focus clears the blur latch); `storage.set` of a `tt:*` record that still does not fit after stripping retries without its own ghost.
